...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExceed> getFilteredWithExceededByStream(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int limitCaloriesPerDay) {
        Map<LocalDate, Integer> caloriesPerDayCount = new HashMap<>();
        List<UserMealWithExceed> filteredMealsWithExceedByTime = new ArrayList<>();
        mealList.forEach(u -> caloriesPerDayCount.merge(u.getDate(),
                u.getCalories(), (oldAmount, newAmount) -> oldAmount + newAmount));
        filteredMealsWithExceedByTime = mealList
                .stream()
                .filter(u -> TimeUtil.isBetween(u.getTime(), startTime, endTime))
                .map(u -> (createUserMealWithExceed(u, caloriesPerDayCount.get(u.getDate()) > limitCaloriesPerDay)))
